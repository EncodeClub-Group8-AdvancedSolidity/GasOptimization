...
import "./lib/YulDeployer.t.sol";
import "../src/Gas.sol";

// cast interface ./out/Gas.sol/GasContract.json
interface Gas {
    ...
    function whiteListStruct(address) external view returns (uint256);
    function whiteTransfer(address _recipient, uint256 _amount) external;
    function whitelist(address) external view returns (uint256);

    function initialize(address[] memory, uint256 _totalSupply) external;
}

contract GasYulTest is Test {
    ...
    function setUp() public {
        ...
        gas = Gas(yulDeployer.deployContract("Gas", owner));

        vm.startPrank(owner);
        gas.initialize(admins, totalSupply);
        vm.stopPrank();
    }
